...
- connect using a Jira token with auth scheme **Auto / Basic / Bearer**
- create a new **Story**, **Task**, or **Epic**
- update an existing Story/Task/Epic selected from a list
- optionally auto-create frontend subtasks for Stories
...
4. (Optional, for teammates on the shared team webhook) Copy `secrets.local.example.js` to
   `secrets.local.js` and fill in the real Slack webhook URL. This file is gitignored - it never
   gets committed - so the real webhook URL never enters this repo's git history. Without it,
   `DEFAULT_SLACK_WEBHOOK_URL` falls back to empty and each person just pastes their own webhook
   URL into Options (Slack Integration section) instead of it being pre-filled.
5. Open extension **Options** and configure:
   - Jira URL
   - Jira token auth scheme (Auto recommended)
   - Atlassian account email (required for Basic mode)
   - Jira token (or capture from clipboard)
   - Run **Auth diagnostics** to verify which scheme/root succeeds (also checks a write/POST call, not just read)
   - Jira API root override - defaults to `/rest/api/2` (the confirmed-working root for Jira
     Server/Data Center); clear it to fall back to auto-detection (mainly for Jira Cloud, which
     uses `/rest/api/3`)
   - Click **Initialize connection** to validate auth + connection
   - Frontend assignee account IDs + roles

## Notes

- Auto mode tries `Basic(email:token)` first (if email is set), then `Bearer token`.
